...
Apache2Redirects.prototype.use = function(req, res, next) {
  if(this.opts.check.header && ( this.opts.check.header(this.headers) || !this.cases.length ) ){
    this.getFile();
  }
  ...
  return Promise.all(promises)
    .then(e => {
      ...
      for (let i = e.length - 1; i >= 0; i--) {
        ...
        this.headers[e[i].name] = e[i].headers;
      }

      this._request = false;
      this.cases = cases.concat(this.opts.source.internal.map(z => ({ ...z, source : 'Internal' })));
    })
};
/**
 * API
